function [A_noS,B_noS,C_noS,D_noS] = build_ABCD_P(V_inf,Structure,Aero)
...
B_noS = [zeros(num_modes),         zeros(num_modes,num_IMU), zeros(num_modes,3*num_AIL);
         eye(num_modes),           zeros(num_modes,num_IMU), Mff_til\Bgx, Mff_til\Bgx_dot, Mff_til\Bgx_ddot;
         zeros(num_x_L,num_modes), zeros(num_x_L,num_IMU),   E_ux_til,    zeros(num_x_L,2*num_AIL)];

%% build_CD ---------------------------------------------------------------
D_til           = repmat(eye(num_modes),1,num_poles);
% D_til           = repmat(speye(num_modes),1,num_poles);

C_noS = [ eye(num_modes), zeros(num_modes), zeros(num_modes,num_x_L);
          zeros(num_modes), eye(num_modes), zeros(num_modes,num_x_L);
          PHIzg*PHIgf*(Mff_til\Kff_til), PHIzg*PHIgf*(Mff_til\Dff_til), PHIzg*PHIgf*(Mff_til\D_til) ];


D_noS = [ zeros(2*num_modes, num_modes), zeros(2*num_modes, num_IMU), zeros(2*num_modes,3*num_AIL);
          PHIzg*PHIgf, eye(num_IMU) ,PHIzg*PHIgf*(Mff_til\Bgx),PHIzg*PHIgf*(Mff_til\Bgx_dot),PHIzg*PHIgf*(Mff_til\Bgx_ddot)];
